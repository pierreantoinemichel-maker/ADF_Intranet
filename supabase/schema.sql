...
create table public.entreprises (
  ...
  zone text, -- 'France' | 'Royaume-Uni' | 'Monde'
  region text,
  metier text, -- 'Pierre & Maçonnerie' | 'Peinture & Décors' | 'Métal' | 'Bois' | 'Décors' | 'Lots techniques'
  metier_label text,
...
-- ──────────────────────────────────────────
-- LIAISON ENTREPRISE <-> SAVOIR FAIRE (n-n)
-- ──────────────────────────────────────────
...
create table public.knowledge_base (
  ...
  description text,
  content text,
  source text, -- nom de l'entreprise source
  entreprise_id uuid references public.entreprises(id) on delete set null,
  created_at timestamptz default now(),
  updated_at timestamptz default now()
);
